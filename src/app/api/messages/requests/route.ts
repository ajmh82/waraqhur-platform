...
export async function GET(request: Request) {
  ...
  try {
    ...
    const where =
      status && status !== "all"
        ? { ...whereBase, status: status as "PENDING" | "ACCEPTED" | "REJECTED" | "CANCELED" }
        : whereBase;
  ...
  try {
    ...
    if (!targetUserId) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "TARGET_USER_REQUIRED", message: "Target user is required" },
        },
        { status: 400 }
      );
    }
    ...
    if (targetUserId === requesterUserId) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "SELF_REQUEST_NOT_ALLOWED", message: "Cannot request yourself" },
        },
        { status: 400 }
      );
    }
